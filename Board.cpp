...
namespace cs32
{
    ...
    bool Board::completed() const {
        for (int i = 1; i <= 9; ++i) {
            for (int j = 1; j <= 9; ++j) {
                if (mCells[i][j].getValue() == 0 || !mCells[i][j].isValueOK(mCells[i][j].getValue(), buildBox(i, j), buildRow(i, j), buildColumn(i, j))) {
                    return false; // Empty cell or invalid value found
                }
            }
        }
  ...
  int Board::evaluate() const {
    ...
    return (correctCount * 100) / 81; // Percentage of correctly filled cells
}
